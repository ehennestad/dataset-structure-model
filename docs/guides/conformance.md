...
- **Ancestors inferred from descendants** — a subject whose id is read from session names in a location without subject folders — get a record with `locations: []`. An ancestor that has a folder somewhere lists only those locations.
- **`metadata`** is the union of the entity's own fields across the locations it appears in, plus the identity fields of its entity-typed ancestors (not their other fields). A field with a rule that matched nothing takes the definition's `defaultValue`; without one it is absent and the record carries `extraction-failed`. A field with no rule in any visited location is absent without an issue.
...
- **`metadata-conflict`**: locations disagree on a field's value; the reader picks one and reports the conflict.
