...
The case exists for one trap. `m110-20250510-0010_raw.tif` differs from `m110-20250510-001_raw.tif` by one digit. A pattern matched by containment puts both under `m110-20250510-001`; a `{token}` or anchored pattern does not. `pathsMustNotShareRecord` fails any config that groups them — this is the `m1001_s1` vs `m1001_s10` confusion the frozen-core decision on file-level grouping was written to prevent.
